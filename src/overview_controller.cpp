...
void OverviewController::handleKeyboard(const IKeyboard::SKeyEvent& event, Event::SCallbackInfo& info) {
    ...
    switch (keysym) {
        ...
        case XKB_KEY_BackSpace:
            if (m_searchActive && !m_searchString.empty()) {
        ...
        default: {
            char buffer[32];
            int  len = xkb_state_key_get_utf8(keyboard->m_xkbState, event.keycode + 8, buffer, sizeof(buffer));
            if (len > 0) {
                for (int i = 0; i < len; ++i) {
                    if (std::isprint(static_cast<unsigned char>(buffer[i]))) {
                        m_searchString += buffer[i];
                        m_searchActive = true;
                    }
                }
                if (m_searchActive) {
...
bool OverviewController::backgroundBlurEnabled() const {
    return getConfigInt(m_handle, "plugin:hymission:background_blur", 1) != 0;
}

double OverviewController::backgroundBlurAlpha() const {
    return std::clamp(getConfigFloat(m_handle, "plugin:hymission:background_blur_alpha", 0.70), 0.0, 1.0);
}

bool OverviewController::showTitlesEnabled() const {
    return getConfigInt(m_handle, "plugin:hymission:show_titles", 0) != 0;
}
...
void OverviewController::beginOpen(const PHLMONITOR& monitor, ScopeOverride requestedScope) {
    ...
    if (m_state.phase == Phase::Inactive) {
        setAnimationsEnabledOverride(false);
    }
...
void OverviewController::renderBackdrop() const {
    ...
    if (!monitor)
        return;
    const float alpha = backgroundBlurEnabled() ? static_cast<float>(backgroundBlurAlpha()) * progress : 0.85F * progress;
    g_pHyprOpenGL->renderRect(
        ...
        {.blur = backgroundBlurEnabled(), .blurA = static_cast<float>(progress)});
}
...
void OverviewController::renderSelectionChrome() const {
    ...
    const bool showTitles = showTitlesEnabled();

    if (showTitles || showFocusIndicator) {
        for (const auto& window : m_state.windows) {
            if (window.targetMonitor != renderMonitor)
                continue;

            const bool isSelected = m_state.selectedIndex && m_state.windows[*m_state.selectedIndex].window == window.window;
            const bool isHovered = m_state.hoveredIndex && m_state.windows[*m_state.hoveredIndex].window == window.window;

            if (showFocusIndicator && isHovered) {
                renderOutline(currentPreviewRect(window), CHyprColor(0.95, 0.97, 1.0, 0.55 * progress), HOVER_THICKNESS);
            }

            if (isSelected) {
                renderOutline(currentPreviewRect(window), CHyprColor(0.24, 0.78, 1.0, 0.95 * progress), OUTLINE_THICKNESS);
            }

            if (showTitles || isSelected) {
                const Rect  rectGlobal = currentPreviewRect(window);
                const Rect  rect = rectToMonitorRenderLocal(rectGlobal, renderMonitor);
                float titleAlpha = isSelected ? 1.0F : (isHovered ? 0.8F : 0.5F);
                if (!showTitles) titleAlpha = 1.0F;

                auto texture =
                    g_pHyprRenderer->renderText(window.title, CHyprColor(1.0, 1.0, 1.0, std::min(1.0, (double)titleAlpha * progress)), scaleFontSizeForRender(renderMonitor, 14), false, "",
                                                static_cast<int>(rect.width));
                if (texture) {
                    const Rect titleRect =
                        makeRect(rect.x + (rect.width - texture->m_size.x) * 0.5,
                                 std::max(scaleLengthForRender(renderMonitor, 8.0), rect.y - texture->m_size.y - scaleLengthForRender(renderMonitor, TITLE_PADDING)),
...
OverviewController::State OverviewController::buildState(const PHLMONITOR& monitor, ScopeOverride requestedScope, const std::vector<WorkspaceOverride>& workspaceOverrides,
    ...
    for (const auto& window : g_pCompositor->m_windows) {
        ...
        if (m_searchActive && !m_searchString.empty()) {
            ...
            std::string lowTitle = asciiLowerCopy(title);
            std::string lowSearch = asciiLowerCopy(m_searchString);
            if (lowTitle.find(lowSearch) == std::string::npos && window->m_szClass.find(lowSearch) == std::string::npos)
                continue;
        }
    ...
    if (preserveExistingOrder && !m_state.windows.empty()) {
        ...
        state.searchActive = m_searchActive;
        state.searchString = m_searchString;
...
void OverviewController::renderSearch() const {
    if (!m_searchActive || m_searchString.empty())
        return;
    ...
    const std::string searchText = "Search: " + m_searchString;
    auto texture = g_pHyprRenderer->renderText(searchText, CHyprColor(1.0, 1.0, 1.0, 0.9 * progress), scaleFontSizeForRender(renderMonitor, 24), false, "");
    if (!texture)
        return;

    const double padding = scaleLengthForRender(renderMonitor, 16.0);
    const Rect boxRect = makeRect((renderMonitor->m_size.x - texture->m_size.x) * 0.5 - padding,
                                  scaleLengthForRender(renderMonitor, 80.0),
                                  texture->m_size.x + padding * 2.0,
                                  texture->m_size.y + padding);
    
    const Rect boxLocal = rectToMonitorRenderLocal(boxRect, renderMonitor);
    g_pHyprOpenGL->renderRect(toBox(boxLocal), CHyprColor(0.1, 0.1, 0.1, 0.6 * progress), {.round = static_cast<int>(boxLocal.height * 0.5)});
    
    const Rect textRect = makeRect(boxLocal.x + padding, boxLocal.y + padding * 0.5, texture->m_size.x, texture->m_size.y);
